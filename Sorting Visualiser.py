...
from heapSort_ import heapSort
root = Tk() 
...
def drawData(data, colourArray):
    ...
    for i, height in enumerate(normalisedData): # enumerate allows us to iterate through a list and track its index simultaneously 
        #top left corner
        x0 = i * x_width + offset + spacing
        y0 = canvas_height - height * 790
        #bottom right
...
UI_frame = Frame(root, width=1920, height=200, bg='grey') # create a frame for the UI to be situated 
UI_frame.grid(row=0, column=0, padx=10, pady=5) #draw, position and pad the grid 

canvas = Canvas(root, width=1920, height=800, bg='white') # create canvas to plot data on 
...
algoMenu = ttk.Combobox(UI_frame, textvariable=selected_aglo, values=['Bubble Sort', 'Merge Sort', 'Insertion Sort', 'Quick Sort', 'Heap Sort']) #dropdown menu 
...
# Row[1]   
sizeEntry = Scale(UI_frame, from_=10, to=150, resolution=1, orient=HORIZONTAL, label="Data Size") # size scale
...
maxEntry = Scale(UI_frame, from_=10, to=150, resolution=1, orient=HORIZONTAL, label="Maximum Value") # Maximum value scale
